#!/bin/bash
# Скрипт для выполнения исправления username на Yandex Cloud VM
...
echo "🔧 Исправление username пользователей на VM: $VM_IP"
echo ""
...
for USER in ubuntu root; do
  for KEY in ~/.ssh/id_rsa ~/.ssh/id_ed25519 ~/.ssh/yandex_key ~/.ssh/event_app_key ~/.ssh/yandex_cloud_key; do
    if [ -f "$KEY" ]; then
      ...
      scp -i "$KEY" -o ConnectTimeout=10 -o StrictHostKeyChecking=no server/scripts/fix-users-usernames.js $USER@$VM_IP:/tmp/ 2>/dev/null && {
...
if [ -f .env ]; then
  ...
  cd /tmp
  node fix-users-usernames.js
  rm fix-users-usernames.js
else
